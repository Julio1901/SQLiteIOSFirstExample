...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func createAndOpenDatabase() -> OpaquePointer? {
        ...
        if let pathComponent = url.appendingPathComponent("FIRSTDB.sqlite"){
    ...
    func createTables() -> Bool {
        ...
        let createTable = sqlite3_exec(dbQueue, "CREATE TABLE IF NOT EXISTS TEMP (TEMPCOLUM1 TEXT NULL, TEMPCOLUM2 TEXT NULL)", nil, nil, nil)
        
        if (createTable != SQLITE_OK) {
            //Log Error here
            print("Erro on create table")
            bRetVal = false
        } else {
            
            //Success scenary
            print("Success on create table")
            bRetVal = true
        }
